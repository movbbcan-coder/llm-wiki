...
| Köprü (VPS) | `/root/oto/backend/` → pm2: `ccoto-api` (127.0.0.1:8787) + `ccoto-bot` (@Ccotobot) |
| Dışa açılım | `https://ccoto.movbbcan.com` (Cloudflare → nginx → 8787), sır başlığı `X-Ccoto-Anahtar` |
| APK | `/root/oto/apk/` → yerel derleme `apk/derle.sh`, kurulum+kapı `apk/kur_ve_dogrula.sh` |
